...
- **Logika Pengingat Waktu Berkala (*Time Reminder*)**: Memeriksa apakah `time_reminder["enabled"]` bernilai `True`. Jika menit saat ini habis dibagi `interval`, berada dalam rentang `start_hour` s/d `end_hour` (dihitung akurat hingga tingkat menit `start_minutes` vs `end_minutes`), dan belum dipicu pada menit tersebut, sistem memformat kalimat berdasarkan `time_format` (12/24 jam) dan `speech_style` yang dipilih (termasuk opsi `follow_f12` yang sinkron dengan `time_settings`) lalu memanggil `tts_manager.speak(msg)` untuk pengucapan mandiri, atau `ui.message()` sebagai fallback.
...
- `TimeReminderDialog`: Form pengaturan pengingat waktu berkala dengan Checkbox Aktifkan, Combo Box untuk Interval, Mode Notifikasi, Gaya Pengucapan Waktu (`speech_style`), Format Jam (`time_format` 12/24 Jam), Jam Mulai, dan Jam Selesai, serta tombol pintas `[ &Pengaturan Suara TTS Mandiri... ]`.
